...
- [ ] Run Loop 48: if explicitly authorized, decide the PR `#182` merge gate after confirming head SHA, mergeability, and checks; stop before production deploy, production probe, provider call, Docker change, or write-path smoke unless separately authorized.
