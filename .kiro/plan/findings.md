...
## 2026-07-22 H1 Knowledge live UAT authorization

- Owner explicitly authorized run `fa-20260722t015126z-3b204a7` against production SHA `3b204a783e555687657ac42f960440002a27ec5e` with model `deepseek-v4-pro`.
- Hard limits: at most four provider calls, `max_output_tokens=900` per call, zero retry, stop on first failure, no DELETE.
- Allowed production writes are limited to at most four `query_logs` rows and corresponding query audit-log rows. All schema/env/runtime/agent/review/document/index/deploy writes remain forbidden.
- Pre-call fail-closed gates must revalidate exact production SHA, model configuration, key SET state, health/readiness and baseline drift. Authorization cannot roll to another SHA, run ID or model.
- The retained one-shot runner and allowlist comparator are intentionally hard-coded to the consumed `fa-20260721t104308z-c5a81923` packet. Before this run they must be copied or narrowly rebound to the newly authorized run ID and SHA; executing the stale constants would violate attribution and authorization.
- The runner already enforces four fixed singleton-collection cases, `deepseek-v4-pro`, one POST per case, stop-on-first-failure, no retry, same-origin HTTPS redirects, no agent invocation, and sanitized evidence output.
- Fresh CLI inspection confirms the production preflight tools support the exact authorized read-only gates: sanitized SSH readiness, GET-only model/deployment catalog, L3 deployment-state health, and serializable read-only guard snapshot capture with an acceptance run ID.
- The live runner and comparator compile after rebinding. Preflight has not yet contacted the provider and no query/audit row has been added for the new identity.
- The first read-only baseline capture rejected the exact authorized run ID as syntactically invalid. Catalog evidence immediately before the rejection passed at the exact production SHA and reported both configured model aliases available. This is a pre-provider tooling blocker, not a provider/UAT failure; fail-closed behavior is active.
- Root cause is deterministic: the authorization used SHA suffix `3b204a7` (7 lowercase hex characters), while the established release-guard identity contract requires `8..32`. `fa-20260722t015126z-3b204a78` satisfies the unchanged validator.
- Current preflight artifacts show readiness `ready_for_smoke`, catalog `pass` at the exact SHA, deployment health `pass`, app/PostgreSQL/ClamAV healthy, embedding count `49051`, and zero health-auditor delta. Baseline is absent because validation stopped before capture.
- Do not weaken the regex or infer permission to replace the run ID. A fresh exact replacement authorization is required; provider call count and new query/audit rows for this attempted packet remain zero by the executed boundaries.
- Owner has now explicitly authorized replacement run ID `fa-20260722t015126z-3b204a78`. This closes the identity blocker without changing the established validator or any other execution boundary.
- Fresh replacement-run evidence confirms no SHA/config/key/catalog/health/baseline drift. Snapshot attribution is empty and the protected-state baseline is internally consistent, so the provider gate is open under the exact authorized packet.
- The deployed parser remediation did not close the provider-side case-2 failure: the new live result reproduced `deepseek_citation_markers_mismatch` after one passing case. The result is not a transport or HTTP failure; the endpoint returned `200`, singleton collection binding held, and the application deliberately used retrieval fallback.
- Fail-stop behavior worked: provider-call upper bound is `2`, retry count is `0`, and no case after the first failure was attempted.
- Boundary evidence is clean despite the functional failure: before/after snapshots passed without concurrent activity; the only deltas are two run-attributable query rows and two matching query audit rows. All prohibited state is unchanged and post-health is green.
- The strongest supported conclusion is `L4 live UAT failed-stopped; L3/allowlist boundary passed`. It is false to claim the deployed citation remediation passed live provider validation.

## 2026-07-22 Loop 62 root-cause trace

- Current `citation_labels_in_text` recognizes bracketed/full-width-parenthesized and bare ASCII `C<number>` labels, case-insensitively, provided they are not embedded in a larger alphanumeric token.
- Therefore the observed reason proves the case-2 normalized answer contained no supported ASCII `C<number>` label at all. It does not prove which unsupported shape, if any, the provider used, nor whether `citation_ids` was missing, empty or populated with available IDs.
- Case 1 passed under the same process, model, config and request contract immediately beforehand. Transport, outer response JSON, inner structured JSON and the deployed parser are therefore functional in at least one same-run response; the remaining fault boundary is case-specific provider output versus the marker contract.
- Existing tests already cover missing/empty metadata with valid visible markers, supported punctuation variants, metadata drift, invalid types, unavailable IDs and completely marker-free answers. No existing deterministic fixture reproduces a safe provider output that the parser incorrectly rejects after the deployed fix.
- A functional parser relaxation is not yet justified. Next inspect retained snapshots and query-log persistence for a sanitized discriminator before choosing observability-only work or stopping without a speculative fix.
- Retained S2 evidence contains only query-log row count/timestamp and primary/content fingerprints; it does not contain stored row bodies. The application persists fallback `answer_summary` and coarse failure reason, not the rejected provider answer or marker-shape diagnostics.
- A local parser probe confirms current accepted forms include `[C1]`, `【C1】`, both parenthesis styles, bare `C1`, and combined `[C1, C2]`. Numeric-only `[1]`/`【1】`, full-width `Ｃ１`, spaced `C 1`, and semantic `引用一` are rejected. The live evidence cannot choose among those rejected forms or no marker at all.
- Current focused provider/citation suite passes `41/41`. There is no existing failing deterministic case after the deployed remediation, so writing a parser RED for any unsupported marker shape would encode a guess rather than reproduce the production failure.
- Fresh official DeepSeek documentation confirms `response_format={"type":"json_object"}` guarantees valid JSON only and relies on prompt examples for the desired fields. Strict JSON-schema conformance exists only for Beta strict tool calls, which is not a drop-in change for the current `/chat/completions` content contract.
- Root-cause conclusion: the production functional failure is provider non-conformance to the visible-marker contract; the precise output shape is unknowable from retained evidence. The repeated blind-remediation cause is diagnostic collapse: both “validated claimed IDs present” and “no claimed IDs” become `deepseek_citation_markers_mismatch`.
- Falsifiable local hypothesis: splitting that reason into safe with-claims/without-claims variants will preserve fail-closed behavior while making the next authorized observation capable of choosing between deterministic rendering architecture and prompt/provider reliability work. No raw answer or secret needs to be retained.
- RED→GREEN confirmed the diagnostic split and allowlist propagation. Full verification passed without changing accepted output behavior.
- Supported local conclusion: the deployed parser is not proven wrong; the provider violated the visible-marker contract on case 2, while retained telemetry was too coarse to say whether valid `citation_ids` existed. The new candidate closes only that telemetry gap.
- Architecture decision after future evidence: with valid claimed IDs, consider application-side deterministic marker rendering under an explicitly approved citation-trust contract; without claimed IDs, treat this as provider/prompt reliability and keep retrieval fallback. DeepSeek Beta strict tool calls are a separate integration, not a transparent JSON-mode switch.

## 2026-07-22 Loop 63 release boundary

- Authorization covers promotion and standard deployment of the observability-only candidate. It does not convert the failed H1 live outcome into a pass and does not authorize another provider call.
- Release acceptance requires exact candidate identity, standard five-part backups, app/static deployment, GET-only smoke and L3 post-state evidence. The diagnostic reason change must not be described as a functional citation repair.
