...
- 已满足：新增 `scripts/run-document-governance-production-readonly-precheck.py` 与 `pnpm document:governance-production-readonly-precheck`，自动复跑非生产 ready-profile 和生产只读准备包，并生成合并 precheck 报告。
- 已满足：precheck 报告输出 `tmp/outputs/document-governance-production-readonly-precheck-latest.json` 与 `.md`，子报告为 `tmp/outputs/document-governance-production-readonly-precheck-ready-profile-latest.json` 和 `tmp/outputs/document-governance-production-readonly-precheck-plan-latest.json`。
- 已满足：本批 precheck 返回 `status=ready_for_manual_authorization_review`、`blockers=[]`，并确认 ready-profile 为 `ready_for_readonly_governance_probe`、生产只读准备包为 `ready_for_production_readonly_plan_review`。
- 已满足：manual authorization todo 已机器生成，覆盖生产 env 名称复核、Tencent COS bucket/region/prefix、对象记录与 retention、脱敏策略版本、审计事件要求、部署 SHA/package checksum、redacted env snapshot 和 rollback 要求。
...
- 已满足：新增 GET-only `GET /documents/governance/status`，并通过 router 前缀自动暴露 `/api/v1/documents/governance/status` 与 `/api/backend/documents/governance/status`；该端点只读取 `ApiState.settings`、schema readiness 状态和 audit log store 配置状态，不读取 upload list，不下载文件，不调用 `record_operation`。
- 已满足：端点返回 storage、COS、governance provider、redaction、audit event contract、`document_storage_objects_schema_ready`、upload-list/download 副作用状态和 audit-log 只读状态的 redacted status；不输出 COS bucket、region、prefix、policy version、env name 或 secret value。
- 已满足：`scripts/run-production-documents-readonly-probe.py` 已加入未来 GET-only `documents-governance-status` 步骤；脚本测试使用 fake HTTP 覆盖该步骤，本批未访问生产。
- 已满足：`pnpm document:governance-production-readonly-coverage` 当前返回 `status=ready_for_governance_config_readonly_probe_with_deploy_metadata_gap`、`evidence_grade=L2-fixture-or-dry-run`，覆盖摘要为 `total=30`、`observable_by_existing_probe=1`、`observable_by_new_governance_status_endpoint=26`、`observable_by_boundary=2`、`not_observable_without_deploy_metadata_endpoint=1`。
- 已满足：目标验证通过：`uv run ruff check src/medical_audit_kb/api/routes_documents.py scripts/prepare-document-governance-production-readonly-observation-coverage.py scripts/run-production-documents-readonly-probe.py tests/knowledge_query/test_api.py tests/knowledge_query/test_scripts.py`；`uv run pytest tests/knowledge_query/test_api.py -k "documents_governance_status"`；`uv run pytest tests/knowledge_query/test_scripts.py -k "production_readonly_coverage or production_documents_readonly_probe"`。
