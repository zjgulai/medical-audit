...
- PR #168 `codex/personal-material-staging-e2e-20260628` 已合入 `main`，merge commit 为 `b625fe79e9fa76e19b27cf671859973222ef2e7c`。
- `main@b625fe79e9fa76e19b27cf671859973222ef2e7c` 已使用部署戳 `personal-material-index-ingestion-main-20260628T072621Z` 发布到生产；本次部署未执行 `--apply-schema`，重建并重启 `medical_audit_app`，不调用生成模型或外部 AI provider。
- 部署前备份已生成：app `/opt/medical-audit/backups/app/pre-deploy-personal-material-index-ingestion-main-20260628T072621Z.tar.gz`，大小 `185121763` bytes；env `/opt/medical-audit/backups/env/medical-audit.env.pre-deploy-personal-material-index-ingestion-main-20260628T072621Z`，大小 `2576` bytes；DB `/opt/medical-audit/backups/db/pre-deploy-personal-material-index-ingestion-main-20260628T072621Z.sql.gz`，大小 `4832928614` bytes；Nginx `/opt/medical-audit/backups/nginx/nginx.conf.pre-deploy-personal-material-index-ingestion-main-20260628T072621Z`，大小 `28485` bytes；Web `/opt/medical-audit/backups/web/audit-web-pre-deploy-personal-material-index-ingestion-main-20260628T072621Z.tar.gz`，大小 `631834` bytes。
- 生产综合 smoke `tmp/outputs/production-e2e-smoke-after-personal-material-index-ingestion-main-20260628T072621Z.json` 为 `status=pass`，覆盖 `9` 个步骤；`query-api-with-citations` 仍返回 `fallback_used=true`，不能解释为真实 no-fallback 生成模型能力上线。
- 生产部署状态审计 `tmp/outputs/tencent-cloud-deployment-state-after-personal-material-index-staging-20260628T072621Z.json` 为 `status=pass`，`issues=[]`，`warnings=[]`；确认远端 `.deploy-sha=b625fe79e9fa76e19b27cf671859973222ef2e7c`、app/postgres/clamav healthy、`audit_next_static_healthy=true`、`audit_mount_present=true`、`matching_embedding_count=49051`。
...
- PR #163 Frontend 2.0 release candidate 已合入 `main`，并使用部署戳 `frontend-2-main-20260628T1142` 发布到生产；该轮生产 `.deploy-sha=0984aad93505cb8eedb36aa8379031c4396b1939`，后续已由 PR #168 部署更新，见上节。
- PR #164 `test(frontend): align production acceptance copy with portal 2.0` 合入点为 `de648cccd855336d850c837fcaf0b5750ba0ede3`，只同步生产前端验收脚本文案；该轮生产 `.deploy-sha` 仍为 PR #163 的 `0984aad93505cb8eedb36aa8379031c4396b1939`。
...
- 个人材料索引 readiness 只读复核 `tmp/outputs/production-personal-material-indexing-readiness-20260628T1625.json` 为 `status=pass`：生产入索引 env 已打开，目标版本为 `personal-materials-cos-staging-pr152-20260619`，当时 `total_uploads=21`、`ready_not_indexed_uploads=2`、`staged_uploads=2`、`personal_material_candidate_versions=1`、`personal_material_chunks=2`、`personal_material_active_versions=0`、`personal_material_active_chunks=0`；该 ready 队列已在 PR #168 staging E2E 后清零，见上节。
...
- F2 真实生成模型 no-fallback gate 最新复核仍未通过：生产只读 gate `tmp/outputs/answer-provider-gate-readiness-production-only-20260628T1610.json` 返回 `status=blocked`，blocker 为 `no-provider-api-key-env-set`；本地 Anthropic smoke `tmp/outputs/answer-provider-smoke-anthropic-20260628T1615.json` 返回 `401 invalid x-api-key`。本轮没有写生产 env、没有生产 provider call，也没有执行 no-fallback 生产 E2E。
- 个人材料写入型 staging 前置检查发现源码/API 边界漂移：PR #168 前的生产 `.deploy-sha=0984aad93505cb8eedb36aa8379031c4396b1939` 已包含 `document_upload_ingestion.py`，但 FastAPI state 未装配 `document_upload_indexer`，且缺少 `/documents/uploads/{upload_id}/index-ingestion` 路由；旧 `/documents/uploads/{upload_id}/index` 只更新上传记录索引状态，不会写入 pgvector candidate staging。该缺口已由 PR #168 部署修复，见上节。
...
- `main@c21d985e6853ffcbd4cb06cdf27deb03ab2861bc` 已使用部署戳 `pr147-personal-material-retrieval-isolation-20260619` 通过 `scripts/deploy-tencent-cloud-production.py --execute` 发布到腾讯云生产环境。
- 本次部署的远端备份阶段在 `REMOTE_BACKUP_TIMEOUT_SECONDS=1200` 后触发完成检查；远端 completion marker 和 app/env/db/nginx/web 备份均存在，脚本继续完成后续 rsync、静态资源同步、`.deploy-sha` 写入、`medical_audit_app` 镜像重建、健康检查和生产 smoke。
- 当前生产业务部署标记 SHA：`c21d985e6853ffcbd4cb06cdf27deb03ab2861bc`，远端 `/opt/medical-audit/app/.deploy-sha` 已由部署状态审计核验。
...
- 本轮写入 E2E 证明：干净文本上传后真实 `clamav-sidecar` 返回 clean，个人材料写入 `document_upload_records`、`document_storage_objects` 和腾讯云 COS；owner 可见本人上传，`department-head` 可按 `read-all` 读取；`dlp-review-required` 与 `manual-index-approval-required` 继续阻断，`indexing_triggered=false`。
...
- PR #141 `codex/formalize-clamav-documents-e2e` 已合并到 `main`，merge commit 为 `237ac3d617b8a3c8593a90c4f454d6745eac5687`；PR #141 将正式脚本、生产写入确认 gate 和本节状态记录合入主线。
- `main@237ac3d617b8a3c8593a90c4f454d6745eac5687` 已使用部署戳 `pr141-clamav-e2e-guard-20260618` 执行生产轻量同步；本轮跳过 web build，跳过 app rebuild/restart，只执行远端备份、文件同步、静态同步、`.deploy-sha` 写入、远程健康检查和生产 smoke。
- PR #141 轻量同步后的生产 smoke `tmp/outputs/production-e2e-smoke-after-pr141-clamav-e2e-guard-20260618.json` 为 `status=pass`，覆盖 `9` 个生产 smoke 步骤；其中 `query-api-with-citations` 返回 `fallback_used=true`，只能证明 fallback/citation 链路可用，不代表生成模型 provider 已成功调用。
- PR #141 轻量同步后首次部署状态巡检 `tmp/outputs/tencent-cloud-deployment-state-after-pr141-clamav-e2e-guard-20260618.json` 出现一次 `nginx-config-test-failed`；随后直接 `docker exec ai_video_nginx nginx -t` 返回成功，重跑巡检 `tmp/outputs/tencent-cloud-deployment-state-after-pr141-clamav-e2e-guard-rerun-20260618.json` 为 `status=pass`、`issues=[]`。
...
- PR #122 `codex/pr121-download-metadata-state-doc-sync` 已合并到 `main`，merge commit 为 `65fc07462fbae73e3b53a41ca797b7c6e170cbce`；PR #124 `codex/docs-only-merge-sha-boundary` 已合并到 `main`，merge commit 为 `5c0d6a85e0a7e27cdcf7e85f1c7a3d87f74eb983`；二者均为 docs-only 状态同步，不代表业务运行代码变更。
...
- `/chat` 保留历史请求别名 `kimi-2.7` 对应 `MEDICAL_AUDIT_KB_CHAT_MODEL_KIMI_2_7_*`，但界面展示实际默认模型 `Kimi K2.6（兼容别名）`；`deepseek-v4-pro` 对应 `MEDICAL_AUDIT_KB_CHAT_MODEL_DEEPSEEK_V4_PRO_*`。
...
- 远端备份阶段必须设置本地超时和远端完成哨兵；如果 SSH 子进程超时，只有在远端 marker 与 app/env/db/nginx/web 备份文件均存在时才允许恢复继续，否则必须失败并进入人工恢复流程。
...
### 5.31 PR #147 个人材料检索隔离生产部署与 `/documents` 写入 E2E

已在 2026-06-19 完成 PR #147 合并、腾讯云生产部署和 `/documents` 写入型 E2E：

- PR #147 merge commit：`c21d985e6853ffcbd4cb06cdf27deb03ab2861bc`；部署戳为 `pr147-personal-material-retrieval-isolation-20260619`。
- 生产部署脚本远端备份阶段达到 `REMOTE_BACKUP_TIMEOUT_SECONDS=1200` 后自动执行完成检查；远端 completion marker 与 app/env/db/nginx/web 备份均存在，脚本继续执行并完成 `.deploy-sha` 写入、app 镜像重建、健康检查和 smoke。
- 当前生产 `.deploy-sha=c21d985e6853ffcbd4cb06cdf27deb03ab2861bc`。
- 写入前 DB 备份：`/opt/medical-audit/backups/db/pre-deploy-pr147-personal-material-retrieval-isolation-20260619.sql.gz`，大小 `1026256364` bytes。
- 应用备份：`/opt/medical-audit/backups/app/pre-deploy-pr147-personal-material-retrieval-isolation-20260619.tar.gz`，大小 `184571281` bytes。
- env 备份：`/opt/medical-audit/backups/env/medical-audit.env.pre-deploy-pr147-personal-material-retrieval-isolation-20260619`，大小 `2205` bytes。
- Nginx 备份：`/opt/medical-audit/backups/nginx/nginx.conf.pre-deploy-pr147-personal-material-retrieval-isolation-20260619`，大小 `30973` bytes。
- Web 静态资产备份：`/opt/medical-audit/backups/web/audit-web-pre-deploy-pr147-personal-material-retrieval-isolation-20260619.tar.gz`，大小 `440131` bytes。
- 生产 smoke：`tmp/outputs/production-e2e-smoke-after-pr147-personal-material-retrieval-isolation-20260619.json`，状态 `pass`，覆盖 `9` 个生产 smoke 步骤。
- 部署状态审计：`tmp/outputs/tencent-cloud-deployment-state-after-pr147-personal-material-retrieval-isolation-20260619.json`，状态 `pass`，`issues=[]`；`app_health=healthy`，`postgres_health=healthy`，`clamav_health=healthy`，`search_backend_ready=true`，`matching_embedding_count=49051`。
- 生产 `/documents` 只读探针：`tmp/outputs/production-documents-readonly-after-pr147-personal-material-retrieval-isolation-20260619.json`，状态 `pass`，确认不产生上传、下载或 provider 调用。
- 生产前端语义验收：`tmp/outputs/production-frontend-acceptance-after-pr147-personal-material-retrieval-isolation-20260619.json`，状态 `pass`，覆盖 `21` 个路由、`42` 个检查，`p0_count=0`，`p1_count=0`。
