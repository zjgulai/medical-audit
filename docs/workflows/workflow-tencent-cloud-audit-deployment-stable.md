...
- 个人材料索引 readiness 只读复核 `tmp/outputs/production-personal-material-indexing-readiness-20260628T1625.json` 为 `status=pass`：生产入索引 env 已打开，目标版本为 `personal-materials-cos-staging-pr152-20260619`，当前 `total_uploads=21`、`ready_not_indexed_uploads=2`、`staged_uploads=2`、`personal_material_candidate_versions=1`、`personal_material_chunks=2`、`personal_material_active_versions=0`、`personal_material_active_chunks=0`；两个 ready/not-indexed 样本均有本地隔离文件，后续如要继续 staging 需单独授权写入型入索引操作。
...
- 个人材料写入型 staging 前置检查发现源码/API 边界漂移：当前生产 `.deploy-sha=0984aad93505cb8eedb36aa8379031c4396b1939` 已包含 `document_upload_ingestion.py`，但 FastAPI state 未装配 `document_upload_indexer`，且缺少 `/documents/uploads/{upload_id}/index-ingestion` 路由；旧 `/documents/uploads/{upload_id}/index` 只更新上传记录索引状态，不会写入 pgvector candidate staging。本地候选 PR 已补齐 indexer wiring、治理角色门禁、`/index-ingestion` API 和显式生产写入确认脚本；合并部署前，生产仍为 `production unchanged`，不得直接把 ready 样本视为已入索引。
