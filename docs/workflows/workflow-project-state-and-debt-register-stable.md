...
- 个人材料上传治理 provider 配置层已完成生产部署；默认生产配置下病毒扫描和 DLP adapter 为 `unconfigured`，会以 `index_readiness` 明确阻断个人材料实际入索引，并返回三项 blockers 供前后端和审计日志消费。本地已新增外部治理 provider pending 语义：配置 `tencent-ci-virus`、`clamav-sidecar`、`ruleset-v1` 或 `external-dlp` 时，响应保留 `blocked`，并用 `result_code=pending-external-result` 表达仍需外部结果写回。
...
- 文档检索个人材料当前已完成留存、角色读取隔离、入索引治理门禁表达、人工审批状态机、腾讯云 COS 对象存储、下载元信息授权隔离和 signed URL 下载交付生产 E2E；外部治理 provider pending 语义已完成本地实现和测试，但尚未完成外部扫描/DLP 结果回写、生产部署或生产写入型 E2E；真实认证、生产级病毒扫描、生产级 DLP/脱敏改写、个人材料实际入索引流程和生产搜索历史列表/回填专项验收仍未完成。
...
| --- | --- | --- | --- | --- | --- | --- |
| P0-01 | 产品集成债务 | 门户核心模块仍以静态数据和本地 state 为主 | `/agents` 和 `/projects` 已完成生产写入验收；`/analytics` 已完成生产上传解析、上传留存和历史记录验收；`/documents` 已完成生产查询、来源集合回显、文档权限接口、个人材料留存写入型验收、上传治理门禁表达验收、人工入索引审批状态机验收、腾讯云 COS 对象存储验收、下载元信息授权隔离验收和 signed URL 下载交付生产 E2E；其余模块仍多依赖 `portal-data` | 页面存在但业务闭环不完整，容易误判为功能已完成 | 下一步补生产级上传文件病毒扫描/DLP/脱敏、真实认证权限、个人材料实际入索引、知识库/图谱/报告/整改页面 API | 新增/查询/刷新后数据仍存在；上传文件可追溯留存、可写入 COS 对象和对象元数据，并通过治理门禁、人工审批状态机、下载元信息权限隔离和授权 signed URL 下载交付；前端测试、API 测试和生产写入/只读验收通过 |
...
| P0-05 | 合规闭环债务 | 证书级电子签章、长期留存介质、生产级病毒扫描和 DLP/脱敏仍未完成 | 当前已有 HMAC 归档签名、本地附件归档、个人材料腾讯云 COS 对象存储、下载元信息授权隔离和 signed URL 下载交付；PR #128 已部署到生产，`tmp/outputs/production-documents-signed-download-e2e-after-pr128-20260618.json` 为 `status=pass`，授权 owner 可取得短期 URL 并完成真实对象下载，越权普通 `auditor` 返回 `404`，审计日志不保存 URL 本体；外部治理 provider pending 语义已完成本地实现和测试，配置外部 provider 时仍 blocked 并返回 `result_code=pending-external-result` | 报告与归档不能作为完整合规交付 | 后续继续设计外部扫描/DLP 结果回写、生产级病毒扫描、DLP/脱敏、下载审计导出、证书级电子签章、长期留存介质和恢复演练方案 | signed URL 授权下载已通过生产 E2E；剩余门禁为病毒扫描/DLP 结果回写、归档包、签章、验签、长期留存介质、恢复演练和真实认证权限通过 |
| P0-06 | 状态源债务 | 本地分支、生产 SHA、远端主线、多个 worktree 容易产生认知漂移 | 生产 `.deploy-sha=b1cd474113dbdb4f58ef59ae32781b06c9387e20`；PR #128 已完成生产部署并通过 smoke、状态审计、前端语义验收和 `/documents` signed URL 生产 E2E；PR #129 为 docs-only 状态同步，合并后不代表生产业务部署前进；生产 `/opt/medical-audit/app/.git` 污染文件已备份并删除；本地仍有多个 worktree 和未跟踪参考目录 | 后续部署可能混入非目标状态 | 后续功能继续从干净 `codex/` 分支切出，部署前核验远端 main、生产 `.deploy-sha`、docs-only/tooling 差异和未跟踪排除清单 | `git status` 清晰；PR、部署 SHA、文档一致；生产目录不残留 worktree Git 指针 |
