...
- 当前生产部署 SHA：`9bc399468a80d540097c16239736ee18bcbcfc27`
...
- 最新文档检索搜索历史写入型 E2E 报告：`tmp/outputs/production-documents-history-write-e2e-20260615.json`，状态 `pass`。
- 最新生产部署状态审计报告：`tmp/outputs/tencent-cloud-deployment-state-after-documents-history-deploy-20260615.json`，状态 `pass`，`issues=[]`。
- 最新生产前端语义验收报告：`tmp/outputs/production-frontend-acceptance-after-documents-history-deploy-20260615.json`，状态 `pass`，`p0_count=0`、`p1_count=0`。
- 最新 AI 数据分析留存历史本地联调截图：`tmp/screenshots/tmp-screenshot-analytics-retention-history-20260615.png`。
- 最新文档检索搜索历史生产浏览器截图：`tmp/screenshots/production-documents-history-e2e-20260615.png`。
...
- 文档检索搜索历史生产结果：`POST /api/v1/query` 写入 `query_logs` 后返回 `query_log_id=e2126ff7-e0d4-4253-8ee9-284ecb3b17a4`；`GET /api/v1/query/logs` 通过 `SqlAlchemyQueryHistoryStore` 从 PostgreSQL 回读最新历史；生产 `/documents` 页面已显示本轮唯一验收问题和 `已连接` 状态。

生产结论：当前生产检索、引用、预览、静态门户、文档检索查询、文档检索搜索历史持久化、任务级复核写入链路、项目成员持久化写入链路、提示词型智能体持久化写入链路、AI 数据分析上传解析链路和 AI 数据分析上传留存/历史记录链路可用；不能据此宣称真实医院审计、真实生成模型、真实权限体系、病毒扫描、脱敏留存、对象存储、文档权限模型或案件级合规闭环已完成。

### 2.2 本地仓库状态

- 当前工作区：`/Users/pray/project/medical_audit`
- 当前本地工作分支：`codex/documents-history-production-sync`
- GitHub `main` 最新提交：`9bc399468a80d540097c16239736ee18bcbcfc27`
- 当前生产运行代码 SHA：`9bc399468a80d540097c16239736ee18bcbcfc27`
- 本轮已完成 PR #81 合并、腾讯云生产部署、`/documents` 生产写入型 E2E、浏览器验收和生产前端 acceptance gate。
- 当前存在额外 worktree：
...
- 文档检索搜索历史持久化已完成生产部署和写入型 E2E；`/api/v1/query` 返回 `query_log_id`，`GET /api/v1/query/logs` 可从 PostgreSQL `query_logs` 读取历史，`/documents` 可展示、刷新和回填历史。
...
- 个人知识库上传、文档权限模型和响应中的 `source_collection` 直接回显仍未完成。
...
- 结果：`p0=[]`，`p1=[]`。
- 最新一次语义验收：`tmp/outputs/production-frontend-acceptance-after-documents-history-deploy-20260615.json`，状态 `pass`；`check_count=42`（21 个路由×2 viewport），`summary.api_checks` 中 `/audit/logs` 与 `/audit/logs/export` 均满足 `denied_status=403`、`allowed_status=200`，`p0=[]`、`p1=[]`。
...
- 本节只记录本地实现验收；生产部署和写入型 E2E 见 2.12。
- 本轮未实现个人知识库上传、文档权限模型或响应中的 `source_collection` 直接回显。

### 2.12 Phase 2.6 文档检索搜索历史生产验收状态

验收日期：`2026-06-15`

本轮 Phase 2.6 已完成生产部署和写入型验收，结论为 `pass`。

生产部署：

- 部署提交：`9bc399468a80d540097c16239736ee18bcbcfc27`。
- 部署戳：`documents-history-20260615`。
- 生产部署前已确认 `public.query_logs` 表存在，本轮未执行额外 schema apply。
- 远端备份覆盖 `app`、`env`、`db`、`nginx` 和 `web`：
...
- 部署后基础 smoke：`tmp/outputs/production-e2e-smoke-after-documents-history-deploy-20260615.json`，状态 `pass`。
- 部署状态审计：`tmp/outputs/tencent-cloud-deployment-state-after-documents-history-deploy-20260615.json`，状态 `pass`，`issues=[]`，远端 `.deploy-sha` 为 `9bc399468a80d540097c16239736ee18bcbcfc27`。
- 生产前端验收：`tmp/outputs/production-frontend-acceptance-after-documents-history-deploy-20260615.json`，状态 `pass`，`route_count=21`、`check_count=42`、`p0_count=0`、`p1_count=0`。
- `/documents` 写入型 E2E：`tmp/outputs/production-documents-history-write-e2e-20260615.json`，状态 `pass`；写入问题 `文档搜索历史生产写入验收 20260615T1138`，返回 `query_log_id=e2126ff7-e0d4-4253-8ee9-284ecb3b17a4`，历史读取后端为 `SqlAlchemyQueryHistoryStore`。
- `/documents` 浏览器验收：`tmp/screenshots/production-documents-history-e2e-20260615.png`；生产页面显示 `搜索历史`、`已连接` 和本轮唯一验收问题，且未显示空状态 `暂无持久化搜索记录。`。

边界：

- 部署脚本 post-check 曾报告共享 Nginx 配置告警 `host not found in upstream "melwater_web"`；该告警来自同机共享入口的其它业务 upstream。随后专用部署状态审计通过，`medical_audit_app`、`medical_audit_pg`、静态挂载和检索后端均为健康状态。
- 本轮完成搜索历史持久化，不等于完成个人知识库上传、文档权限模型或响应中的 `source_collection` 直接回显。
- 普通 query smoke 仍允许 `fallback_used=true`，不证明真实生成模型能力可用。
...
| --- | --- | --- | --- | --- | --- | --- |
| P0-01 | 产品集成债务 | 门户核心模块仍有多处静态数据和只读壳层 | `/agents` 和 `/projects` 已完成生产写入验收；`/analytics` 已完成生产上传解析、上传留存和历史记录验收；`/documents` 已完成生产查询与搜索历史持久化写入验收；其余模块仍多依赖 `portal-data` | 页面存在但业务闭环不完整，容易误判为功能已完成 | 下一步补个人知识库上传、文档权限模型、上传文件病毒扫描/脱敏/对象存储治理、知识库/图谱/报告/整改页面 API | 新增/查询/刷新后数据仍存在；上传文件可追溯留存并通过治理门禁；前端测试、API 测试和生产写入验收通过 |
...
| P0-06 | 状态源债务 | 本地分支、生产 SHA、远端主线、多个 worktree 容易产生认知漂移 | 本轮已将生产 SHA、远端 main 和文档状态同步到 `9bc399468a80d540097c16239736ee18bcbcfc27`；本地仍有多个 worktree 和未跟踪参考目录 | 后续部署可能混入非目标状态 | 后续功能继续从干净 `codex/` 分支切出，部署前核验远端 main、生产 `.deploy-sha` 和未跟踪排除清单 | `git status` 清晰；PR、部署 SHA、文档一致 |
...
4. 文档检索 API-first 接入：生产查询验收和搜索历史持久化写入型 E2E 已完成；个人知识库上传、权限模型和来源集合回显待后续阶段。
