...
| P0-05 | 合规闭环债务 | 证书级电子签章、长期留存介质、生产级病毒扫描和 DLP/脱敏仍未完成 | 当前已有 HMAC 归档签名、本地附件归档、个人材料腾讯云 COS 对象存储、下载元信息授权隔离和 signed URL 下载交付；PR #128 已部署到生产，`tmp/outputs/production-documents-signed-download-e2e-after-pr128-20260618.json` 为 `status=pass`，授权 owner 可取得短期 URL 并完成真实对象下载，越权普通 `auditor` 返回 `404`，审计日志不保存 URL 本体；外部治理 provider pending 语义和结果回写接口已完成本地实现和测试，配置外部 provider 时仍 blocked，并可由授权角色回写 `virus-scan` 或 `dlp-review` 结果 | 报告与归档不能作为完整合规交付 | 后续继续设计生产部署、生产写入型 E2E、真实外部 provider 调用、下载审计导出、证书级电子签章、长期留存介质和恢复演练方案 | signed URL 授权下载已通过生产 E2E；剩余门禁为外部扫描/DLP 生产验收、归档包、签章、验签、长期留存介质、恢复演练和真实认证权限通过 |
