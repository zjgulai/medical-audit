...
生产结论：当前生产检索、引用、预览、静态门户、任务级复核写入链路、项目成员持久化写入链路和提示词型智能体持久化写入链路可用；不能据此宣称真实医院审计、真实生成模型、真实权限体系或案件级合规闭环已完成。
...
| --- | --- | --- | --- | --- | --- | --- |
| P0-01 | 产品集成债务 | 门户核心模块仍以静态数据和本地 state 为主 | `/agents` 和 `/projects` 已完成生产写入验收；`/analytics` 和 `/documents` 仍需补生产级验收；其余模块仍多依赖 `portal-data` | 页面存在但业务闭环不完整，容易误判为功能已完成 | 继续补表格上传生产验收、文档检索生产业务验收和知识库/图谱/报告/整改页面 API | 新增/查询/刷新后数据仍存在；上传解析和文档检索走后端；前端测试和 API 测试通过 |
