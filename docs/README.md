...
本目录是 `AI 审计一体化协作平台` 的权威文档入口。外部观察时间为 2026-08-22 00:59（Asia/Shanghai）：Draft PR [#275](https://github.com/zjgulai/medical-audit/pull/275) 的 base 为 `ccc73e95820e39559430e96c01d52c8dfb77a246`，观测 head 为 `d1973206d4f9b01ad0b287fb252fccf760fdab5c`；[exact-head CI run 32499803192](https://github.com/zjgulai/medical-audit/actions/runs/32499803192) 成功，Python `1018 passed`、Web `419 passed`，文档门禁 0 error、0 warning。PR 当时为 `OPEN/DRAFT`、`MERGEABLE/CLEAN`，review、review request 和 review thread 均为 0；CodeRabbit 因 Draft 跳过 review。该观察不等同于 tracked 文档自身身份，也不证明候选已经 Ready、合并或部署。2026 年 8 月 12 日的 L3 只读证据显示，生产当时运行 `25e1654e0c44ca5cbb2bb42e82debdb40fa6f224`；本轮没有刷新生产状态。
...
- `fact`：外部观察显示 PR #275 的 `d1973206…` exact-head CI 已通过 Python `1018` 项、Web `419` 项测试和文档门禁；CodeRabbit status 只证明 Draft 时跳过 review。
- `fact`：当前 tracked 文档采用带时间的非自指观察；文档同步后的最终 exact head 和 CI 必须从 PR、Actions run 与仓库外收据解析。
