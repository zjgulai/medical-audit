...
- 门户核心模块的后端持久化和真实业务闭环：`/projects` 新增成员已完成生产持久化写入验收；`/agents` 生产 API 可读但尚未完成生产写入型 E2E；`/analytics` 仍需生产上传 E2E、上传留存和历史分析记录；`/documents` 仍需生产业务检索验收和搜索历史持久化。
...
V0.3 门户壳层已经部署到生产，项目成员管理已完成生产持久化写入验收。下一阶段不再继续盲目补页面，而是继续处理产品集成债务：补齐提示词型智能体生产写入验收、上传表格分析生产验收、文档检索生产业务验收，并继续把现有审计证据链能力统一到可验收门户。
...
- 收敛 V0.3 产品集成债务：智能体 CRUD 生产写入验收、上传表格分析后端生产验收、文档检索生产业务验收、关键门户页面 API 化和项目成员权限治理。
