...
- AI 数据分析上传留存和历史记录已完成本地实现：新增 `analytics_upload_records`、受控上传文件留存、`GET /api/v1/analytics/table-uploads` 历史列表和 `/analytics` 上传历史侧栏，本地前后端联调通过。

当前未完成：

- 门户核心模块的后端持久化和真实业务闭环：`/agents` 和 `/projects` 已完成生产持久化写入验收；`/analytics` 已完成生产瞬时上传解析验收，上传留存/历史记录已完成本地实现但仍需生产 schema apply、部署和写入型 E2E，病毒扫描和脱敏留存仍未完成；`/documents` 已完成生产查询验收，但仍需搜索历史持久化、个人知识库上传和文档权限模型。
...
V0.3 门户壳层已经部署到生产，提示词型智能体、项目成员管理、AI 数据分析瞬时上传解析和文档检索生产查询已完成生产验收。AI 数据分析上传留存/历史记录已完成本地实现。下一阶段不再继续盲目补页面，而是继续处理产品集成债务：先完成上传留存的生产部署和写入验收，再补文档搜索历史/个人知识库/权限模型，并继续把现有审计证据链能力统一到可验收门户。
...
- 收敛 V0.3 产品集成债务：上传留存/历史记录生产验收、文档搜索历史/个人知识库/权限模型、关键门户页面 API 化、智能体提示词版本治理和项目成员权限治理。
