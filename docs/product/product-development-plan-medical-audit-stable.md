...
- 门户核心模块的后端持久化和真实业务闭环：`/agents` 新增智能体仍只写前端 state，`/projects` 新增成员仍只写前端 state，`/analytics` 只完成浏览器本地 CSV 预检，XLSX 和正式工作簿解析仍等待后端分析服务。
...
V0.3 门户壳层已经部署到生产。下一阶段不再继续盲目补页面，而是先处理产品集成债务：把提示词型智能体、上传表格分析和项目成员管理从静态 UI 或浏览器本地状态升级为后端持久化业务模块，并继续把现有审计证据链能力统一到可验收门户。
...
- 收敛 V0.3 产品集成债务：智能体 CRUD、项目成员管理、上传表格分析后端、关键门户页面 API 化。
