...
- `AI智能审计管理系统` 门户化前端、左侧 9 模块导航、顶部多标签和历史对话区。
- 提示词型智能体列表、智能体广场和新增智能体。
- 知识库三类只读展示、参考系统式文档检索首页、上传表格 AI 数据分析、知识图谱入口、底稿/报告首页和项目成员管理 UI。
...
下一阶段先执行 V0.3 前端门户化改版。核心目标不是泛 UI 打磨，而是把已确认的参考系统式信息架构落成 `AI智能审计管理系统`：9 模块导航、顶部多标签、提示词型智能体、文档检索、上传表格 AI 数据分析、项目成员管理和现有审计证据链能力统一到一个门户。

门户化改版完成后，继续把项目从“知识库支撑层”推进到“单院 HIS 专项审计 MVP”。

阶段目标：

- 固化当前知识库查询引擎为可回滚、可验收、可复测的 V0.2 基线。
- 完成 V0.3 门户化前端改版：`AI智能审计管理系统` 品牌、9 模块导航、顶部多标签、提示词型智能体、文档检索首页、上传表格分析、项目成员管理。
