...
- 门户核心模块的后端持久化和真实业务闭环：`/agents` 和 `/projects` 已完成生产持久化写入验收；`/analytics` 已完成生产瞬时上传解析验收，但仍需上传留存、病毒扫描、脱敏留存和历史分析记录；`/documents` 仍需生产业务检索验收和搜索历史持久化。
...
V0.3 门户壳层已经部署到生产，提示词型智能体、项目成员管理和 AI 数据分析瞬时上传解析已完成生产验收。下一阶段不再继续盲目补页面，而是继续处理产品集成债务：补齐文档检索生产业务验收、上传留存/历史记录，并继续把现有审计证据链能力统一到可验收门户。
...
- 收敛 V0.3 产品集成债务：文档检索生产业务验收、上传留存/历史记录、关键门户页面 API 化、智能体提示词版本治理和项目成员权限治理。
