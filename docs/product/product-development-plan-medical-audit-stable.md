...
- 文档检索搜索历史持久化已完成本地实现和联调：`/api/v1/query` 返回 `query_log_id`，`GET /api/v1/query/logs` 可从 `query_logs` 持久化读取历史，`/documents` 可展示、刷新和回填搜索历史；当前证据目录未闭合生产专项验收，不能据此宣称生产搜索历史列表/回填已完成。
...
- 门户核心模块的后端持久化和真实业务闭环：`/agents` 和 `/projects` 已完成生产持久化写入验收；`/analytics` 已完成生产上传解析、上传留存和历史记录验收，但病毒扫描、脱敏留存、对象存储、下载权限隔离和正式工作簿治理仍未完成；`/documents` 已完成生产查询验收，搜索历史持久化已完成本地实现但生产专项证据未闭合，仍需个人知识库实际入索引、生产搜索历史列表/回填专项验收和真实权限模型。
...
V0.3 门户壳层已经部署到生产，提示词型智能体、项目成员管理、AI 数据分析上传解析、上传留存/历史记录和文档检索生产查询已完成生产验收；文档搜索历史已完成本地实现，但当前证据目录未闭合生产专项验收。下一阶段不再继续盲目补页面，而是继续处理产品集成债务：优先推进个人知识库实际入索引、生产级 DLP/脱敏、AI 数据分析上传治理、文档权限模型和必要的搜索历史生产专项复验，把现有审计证据链能力统一到可验收门户。
...
- 收敛 V0.3 产品集成债务：个人知识库实际入索引、文档权限模型、AI 数据分析上传治理、生产级 DLP/脱敏、搜索历史生产专项复验、关键门户页面 API 化、智能体提示词版本治理和项目成员权限治理。
