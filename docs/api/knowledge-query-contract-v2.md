...
- `generation_failure_code`: a sanitized reason code or `null`; `provider_abstention` records an
  explicit insufficient-evidence refusal; never a provider response body
- `generation_failure_reason`: an allowlisted fallback subtype for malformed responses or explicit
  provider abstention, or `null`; never raw provider content
