...
const outDir = path.resolve(
  packageRoot,
  process.env.OUT_DIR ?? path.join("output", "playwright", `replica-prodlike-smoke-${stamp}`)
);
...
function startProdLikeServer() {
  return spawn("corepack", ["pnpm", "exec", "next", "start", "--port", port], {
    cwd: packageRoot,
    stdio: "inherit",
    env: process.env
  });
...
async function writeSummary(status, error) {
  ...
  const interaction = existsSync(interactionJson)
    ...
    : null;
  const summary = {
    task: "replica-prodlike-smoke",
    status,
    baseUrl,
    outDir,
    boundaries: {
      evidence_grade: "local_production_style_validation",
...
async function main() {
  ...
  const server = startProdLikeServer();
  let failed = null;
  try {
